...
We note that the proof of the proposition is constructive, so we can obtain
the test taker's strategy corresponding to a particular flow.
Subsequently, we can do binary search on $U$ to the desired level of
...
Edmonds-Karp [cite] and Push-Relabel [cite] max flow algorithm respectively
(with binary search) on the
network defined in flow algorithm respectively
