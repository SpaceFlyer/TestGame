...
\centering
	...
	\caption{Runtime for solving an optimal tester strategy in one-question tests.}
\end{figure*}
...
\begin{figure*}
	...
	\caption{Utility increase of optimal test strategies compared
	to the uniform test strategies. Let the optimal utility be
	$u^*$ and the uniform test's utility be $u_0$, the increase
	is $u^*/u_0-1$. The parameter $m_{\text{max}}$ is denoted as $m$
	for short in the main title of each graph.}
	\label{fig:utility}
\end{figure*}

As Figure~\ref{fig:utility} indicates, the increase drops when $L$ and $m$
increase. And it increases when $n$ and $t$ increase.  This is due to the way
we generate the test game instances. Large $L, m$ make each subset of questions
more likely to be equivalent to the tester while large $n, t$ make them more
different.

Note that the way we construct the test game instances, which assumes that each
question has the same probability to be hard for a test taker, favors the
uniform strategies. In realily, it's more likely to have some very hard
