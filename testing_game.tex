...
Given an objective utility $U$, we can check its feasibility and compute
a solution if feasible by constructing a network with graph $(V, E)$ and
capacity $c$ as follows (see also Figure~\ref{fig:network}).  Let $V = \{s\}
\cup \mathbb B \cup A \cup \{t\}$, $E = \{s\} \times \Theta \cup \Theta \times
Q \cup Q \times \{t\}$.  For each edge $(s, \theta)$, let its capacity $c_{(s,
\theta)} = p(\theta) v_\theta m_\theta$. For each edge $(\theta, q)$, let its
capacity $c_{(\theta, q)} = p(\theta) v_\theta$ if $q \in H_\theta$ and $0$
otherwise. For each edge $(q, t)$, let its capacity $c_{(q, t)} = \max(0,
U_q^0-U)$.
...
c_{(\theta, q)}$. To minimize our $U$, we can either do binary search for a
...
\begin{algorithm}
\caption{Compute the optimal test strategy with size one, given the optimal
...
%	z_{\theta,q}$
	...
	\While{$S$ has an unmarked element}
		\State $\theta \gets $ get and mark an unmarked element from $S$
		\ForAll{$q \in H_\theta \cap T$ and $z_{\theta,q} < 1$}
			\State $T \gets T \setminus \{q\}$\label{line:delete-a}
			\State $S \gets S \cup \{\theta': q \in H_{\theta'} \wedge z_{\theta', q}>0\}$
...
We prove the correctness of Algorithm~\ref{alg:one-problem} by firstly
introducing the following two lemmas.
...
problems in $T_0$ are deleted, then for every $q$ in $T_0$ there is a $\theta$
that is in $S$ and $z_{\theta,q} < 1$.  Note that for every $\theta$ in $Q$, it
...
Let $z_{\theta,q}$ be the solution of LP~(\ref{eqn:one-problem}) and $T$ be the
final subset that Algorithm~\ref{alg:one-problem} returns. Then for each type
$\theta$, either $\sum_{q \in H_\theta \cap T} z_{\theta,q} = m_\theta$ (they
always memorize the maximal subset of $T\cap H_\theta$) or $\forall q \in
H_\theta \cap T, z_{\theta, q} = 1$ (they memorize any question in $T\cap
H_\theta$ with probability $1$ so they will pass the test for sure). Therefore
all test takers are best responding.
\end{lemma}
...
$T$ uniformly. Then since memorizing strategy $z_{\theta,q}$ makes $U_q$ (the
