...
\begin{abstract}
Conventionally, tests are assumed to be confidential where problems, answers
and other test related information are kept unkown from test takers.  That
confidentiality, however, becomes very hard to achieve because of the Internet.
For example, we can easily find TOEFL iBT and driver's license tests' problems
that are anonymously posted online by previous test takers. In this paper, we
take the loss of confidentiality as a fact and study the optimal
confidential-free test.  We model it as a Stackelberg game with the tester as a
leader who reveals his test strategy, problems and answers to test takers. The
only limit of a test taker is memorize capacity which means he can only
memorize some of the problems' answers. We model and characterize hardness of
such test games in general. We also give efficient algorithms for cases when
test games are not hard.
\end{abstract}
